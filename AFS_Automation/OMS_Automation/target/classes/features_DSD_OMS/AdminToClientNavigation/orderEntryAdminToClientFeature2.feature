Feature: scenarios for admin credentials and client credentials
  ...
  @PreventOrderSubmissionEnableCancel
  Scenario Outline: Test scenario for verifying admin setting Prevent Order Submission enabled
    ...
    Examples:
      |  Setting                                                             |          Key                |
      | Hard hold - prevent order submission if below minimum amount         | CPHardHoldOrderMinimum      |
