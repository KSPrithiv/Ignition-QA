...
public class OrderEntryPage
{
    ...
    public void ChangeAccount()
    {
        ...
        try
        {
            ...
            act.moveToElement(WebEle).click().build().perform();
            if(HelpersMethod.JSGetValueEle(driver,WebEle,10000).equals(""))
            {
                act.sendKeys(WebEle, Acc_No).build().perform();
            }
            else
            {
                HelpersMethod.JSSetValueEle(driver,WebEle,10000,"");
                WebEle.sendKeys(Keys.BACK_SPACE);
                act.sendKeys(WebEle, Acc_No).build().perform();
            }

            if(HelpersMethod.IsExists("//div[contains(@class,'k-list-container ')]/descendant::li",driver))
            {
            ...
            WebEle = HelpersMethod.FindByElement(driver, "id", "customerAccountNumberComboBox");
            Acc_No = HelpersMethod.JSGetValueEle(driver, WebEle, 10000);
    ...
    public void Change_OldAccount(String account)
    {
        ...
        try
        {
            ...
            new WebDriverWait(driver, Duration.ofMillis(10000)).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[contains(@class,'k-list-container ')]/descendant::li")));
   ...
   public void Refresh_Page1()
    ...
    public void Refresh_Page2()
    {
        ...
        try
        {
            ...
            //If scenarios are failing in Order history page it should navigate back to order entry page
            if(HelpersMethod.IsExists("//div[@id='order-history-card']",driver))
            {
                ...
                {
                    WebElement discardButton=HelpersMethod.FindByElement(driver,"xpath","//div[contains(text(),'Your order has not been submitted')]/ancestor::div[@class='k-window k-dialog']/descendant::button/span[text()='Discard all']");
                ...
                {
                    WebElement pendingPopup=HelpersMethod.FindByElement(driver,"xpath","//div[contains(text(),'Your order has not been submitted')]/ancestor::div[contains(@class,'k-window k-dialog')]");
                    WebElement dismissAll= pendingPopup.findElement(By.xpath("//span[text()='Save order without submitting']/ancestor::button"));
                    HelpersMethod.ActClick(driver,dismissAll,10000);
                }
...
//                    HelpersMethod.ActClick(driver,accNo,10000);
            ...
            PopUps_After_AccountChange();
            WebEle = HelpersMethod.FindByElement(driver, "id", "customerAccountNumberComboBox");
            String Acc_No = HelpersMethod.JSGetValueEle(driver, WebEle, 10000);
            scenario.log("CUSTOMER ACCOUNT# SELECTED IS " + Acc_No);
            if (!Acc_No.equals(null))
            {
                exists = true;
    ...
    public boolean Account_Zero()
    {
        try
        {
            ...
            String Acc1=HelpersMethod.JSGetValueEle(driver,AccNo,10000);
            if(Acc1.contains("-"))
            {
    ...
    public void CustomerIndexPopup() throws InterruptedException
    {
        ...
        HelpersMethod.JScriptClick(driver, AddFilterButton, 10000);

        //Identify the dialog popup
        new WebDriverWait(driver, Duration.ofMillis(10000)).until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(@class,'k-child-animation-container')]")));
        new WebDriverWait(driver,Duration.ofMillis(10000)).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class,'k-child-animation-container')]")));
        ...
        HelpersMethod.EnterText(driver, Search1, 10000, "Customer #");
        ...
        //Click on Check box
        new WebDriverWait(driver, Duration.ofMillis(10000)).until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@id='CM_CUSTKEY']")));
        ...
        new WebDriverWait(driver, Duration.ofMillis(10000)).until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//div[contains(@class,'k-child-animation-container')]/descendant::form[contains(@class,'i-filter-popup')]")));
...
//            System.out.println(formattedDate1);
         ...
         /*   if(HelpersMethod.IsExists("//td[contains(@style,'opacity:')]/span[@class='k-link' and contains(@title,'" + formattedDate1 +"')]",driver))
            ...
            List<WebElement> dates=HelpersMethod.FindByElements(driver,"xpath","//tr[@class='k-calendar-tr']/td[contains(@style,'background-color: rgb(151, 199, 119)')]/span");
            if((dates.size())==1)
            {
            ...
            else
            {
                ...
                {
                    date1 = dates.get(j);
                    Date d2 = dateFormat.parse(date1.getAttribute("title"));

                    if(d1.before(d2))
                    {
                        scenario.log("SELECTED DELIVERY DATE IS "+date1.getAttribute("title"));
                        break;
                    }
                }
                String selectDate=dates.get(j).getAttribute("title");
                WebElement deliveryDate=HelpersMethod.FindByElement(driver,"xpath","//tr[@class='k-calendar-tr']/descendant::span[@title='"+selectDate+"']");
    ...
    public void selectDateForSkipDate(int days)
    {
        ...
        try
        {
            new WebDriverWait(driver,Duration.ofMillis(10000)).until(ExpectedConditions.refreshed(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(@class,'k-datepicker-popup')]"))));
            new WebDriverWait(driver,Duration.ofMillis(10000)).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class,'k-datepicker-popup')]")));
...
//            DateTimeFormatter myFormatObj = DateTimeFormatter.ofPattern("EEEE, MMMM d, yyyy");
//            formattedDate1 = dateTime1.format(myFormatObj);
//            scenario.log("DELIVERY DATE TO BE SET TO "+formattedDate1);
    ...
    public boolean Start_Order()
    {
        ...
        try
        {
            ...
            if (HelpersMethod.IsExists("//button[@id='addButton']", driver) && !HelpersMethod.IsExists("//button[@id='addButton' and contains(@class,'disabled')]",driver))
                {
                ...
                else
                {
                   ...
                   HelpersMethod.ScrollElement(driver, SearchOrder);
                    if (HelpersMethod.IsExists("//button[@id='addButton']", driver) && !HelpersMethod.IsExists("//button[@id='addButton' and contains(@class,'disabled')]",driver))
                    {
                        StartOrder=HelpersMethod.FindByElement(driver,"id","addButton");
                        HelpersMethod.ClickBut(driver, StartOrder, 10000);
                        scenario.log("START BUTTON CLICKED");
                        exists = true;
                    }
                   status = HelpersMethod.returnDocumentStatus(driver);
                   if (status.equals("loading"))
    ...
    public void ClickRemoveSkip()
    {
        ...
        try
        {
            ...
            if (HelpersMethod.IsExists("//button/span[text()='Remove Skip']", driver))
            {
                ...
                scenario.log("REMOVE SKIP BUTTON FOUND");
                exists = true;
            }
            wait = new FluentWait<WebDriver>(driver)
                    .withTimeout(Duration.ofSeconds(600))
...
//            scenario.log("FIRST DATE FOR PICKUP ORDER, IN POPUP IS " + CurrentDate);
    ...
    public void selecteCommerceOption() throws InterruptedException
    {
        ...
        HelpersMethod.ActClick(driver, questionMark, 40000);
        Thread.sleep(2000);

        //verify existence of eCommerce link
        if (HelpersMethod.IsExists("//div[contains(@class,'help-links-line ecommerce-link')]",driver))
        {
...
//            driver.switchTo().window(ParentWindow);
    ...
    public void clickQuestionMark()
    {
        try
        {
            ...
            HelpersMethod.WaitElementPresent(driver, "xpath", "//div[@class='k-child-animation-container')]", 10000);
        }
        catch (Exception e){}
    }
...
//            }
    ...
    public void validateSucessOrErrorSubmittingMessage() throws InterruptedException
    {
        exists=false;
        Wait<WebDriver> wait;
        try
        {
            ...
            CheckOutSummaryPage summary = new CheckOutSummaryPage(driver,scenario);
            for(int i=0;i<=2;i++)
            {
...
//        catch (Exception e){}
    ...
    public void nonExistenceOfSelecteCommerceOption()
    {
        try
        {
            String status = HelpersMethod.returnDocumentStatus(driver);
    ...
    public void selectSaleshelpDisable()
    {
        try
        {
            if (HelpersMethod.IsExists("//div[@class='loader']", driver))
            ...
            HelpersMethod.ActClick(driver, questionMark, 10000);
            HelpersMethod.WaitElementPresent(driver, "xpath", "//div[contains(@class,'k-popup k-child-animation-container k-slide-down-enter k-slide-down-enter-active')]", 600);
            if(!HelpersMethod.IsExists("//div[contains(@class,'help-links-line sales-link')]",driver))
            {
               exists=true;
            }
            Assert.assertEquals(exists, true);
    ...
    public void deliveryDateSelectedIs()
    {
        try
        {
    ...
    public void enabledWarehouse()
    {
        exists=false;
        try
        {
            if(HelpersMethod.IsExists("//div[@data-test-id='WarehouseIndex']",driver))
            {
                scenario.log("WAREHOUSE INDEX BOX IS DISPLAYED");
                exists=true;
            }
            Assert.assertEquals(exists,true);
        }
    ...
    public void disabledWarehouse()
    {
        ...
        try
        {
            if(!HelpersMethod.IsExists("//div[@data-test-id='WarehouseIndex']",driver))
            {
                scenario.log("WAREHOUS INDEX BOX IS NOT DISPLAYED");
                exists=true;
            }
            Assert.assertEquals(exists,true);
        }
